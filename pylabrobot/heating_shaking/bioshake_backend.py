...
class BioShake(HeaterShakerBackend):
  ...
  async def _send_command(self, cmd: str, delay: float = 0.5, timeout: float = 2):
    try:
      ...
      # Other valid responses (for example temperature query values)
      return decoded
  ...
  async def set_temperature(self, temperature: float):
    ...
    # Check if temperature is an integer in 1/10 C units.
    ...
    if not isinstance(temperature, int):
      raise TypeError(
        "Temperature must be an integer or a whole number float, not "
        f"{type(temperature).__name__} (1/10 C)"
      )

    await self._send_command(cmd=f"setTempTarget{temperature}", delay=0.2)
